...
Yes they are similar! They are quite close compared to the real values. Note that the standard errors for the collinear variables are a bit higher compared to the rest. 
...
We want to standardize the predictors for lasso so that all variables are penalized the same despite what "units" they are in. 
...
We do not one to "overfit" the penalty value. The BEST penalty value might be overfit to a particular CV fold and we do not want to use that same penalty value on other sets of data. 
...
They are still very accurate.

5. What happens to the non-informative (value 0) variables? 

The non-informative variables are all set to 0! 
...
LASSO in this case was slightly worse compared to OLS. This may be because of chance. This also might be because we are limited in the number of variables we are using. 
...
Coefficients all begin to decrease towards zero. It takes longer for V6 to decrease to zero than other variables. 

8. V6 is a very important predictor -- how can you see that in this graph? 

At large lambda values, V6 is the only variable standing. This is because V6 has the biggest variance and thus will give the biggest indicator/signal for lasso to capture. 
...
OLS tends to be less biased but more variable. LASSO tends to be more variable but less biased. 
...
LASSO seems to be both less biased and less variable.
