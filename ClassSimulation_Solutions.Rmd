...
We want to standardize the predictors for lasso so that all variables are penalitzed the same despite what "units" they are in. 
...
We do not one to "overfit" the penalty value. The BEST penalty value might be over fitted to a particular CV and we do not want to use that same penalty value on other sets of data. 
...
LASSO in this case was slightly worst compared to OLS. This may be because of chance. This also might be because we are limited to the number of variables we are using. 
...
OLS tends to be less bias but more variance. While LASSO tends to be more varied but less bias. 
...
LASSO seems to be both less bias and less varied 
