...
Feel free to adjust the coefficient values as you like below (Make sure to have some 0's): 
...
We fit many penalty values, which one do you think we will end up picking? Why would we might one to pick the best penalty within 1 se of the lowest CV MAE?
...
```{r}
combined %>% 
  select(V6_lasso, V6_lm) %>% 
  melt() %>% ggplot(aes(x=value, fill = variable))+
  geom_density(alpha = 0.5) + 
  geom_vline(xintercept = V6Coef, linetype="dotted", 
                color = "blue", size=1.5)
```
